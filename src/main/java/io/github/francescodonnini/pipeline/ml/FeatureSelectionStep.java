...
public class FeatureSelectionStep implements Step<MLWorkloadInfo, MLWorkloadInfo> {
    ...
    public MLWorkloadInfo execute(MLWorkloadInfo input) throws Exception {
        ...
        logger.log(Level.INFO, "Selected attributes: {0}", String.join(",", Arrays.stream(selected).mapToObj(trainingSet::attribute).map(Attribute::name).toList()));
        return input;
    }
